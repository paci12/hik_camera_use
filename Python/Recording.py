...
# 为线程定义一个函数
def work_thread(cam=0,flag=None):
...
def record(flag):
    ...
    stRecordPar.enPixelType = c_long(17301505)
    ...
    stRecordPar.strFilePath= './videos/temp.avi'.encode('ascii')
    ...
    try:
        hThreadHandle = threading.Thread(target=work_thread, args=(cam, flag))
        hThreadHandle.start()
    except:
